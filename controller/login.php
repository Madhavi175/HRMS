<?php
	...
	else if (isset($_POST['usave']))
	{
		...
		else
		{
			...
			$sql = mysqli_query($db,"select * from employees_data where Password='$old' and Email='$email'");
	...
	else if (isset($_POST['save']))
	{
		...
		else
		{
			...
			$sql = mysqli_query($db,"select * from admin_data where Password='$old' and Email='$email'");
			if(mysqli_num_rows($sql) > 0)
			{
				if(!empty($npass) && !empty($cpass))
				{
					if($npass == $cpass)
					{
						mysqli_query($db,"update admin_data set Password='$npass' where Password='$old' and Email='$email'");
	...
	else if(isset($_POST['submit']))
	{
		...
		{
			$sql = mysqli_query($db,"select * from admin_data where Email='$Username' AND Password='$Password' AND id=1 ");
	...
	function user()
	{
		...
		$mac = $GLOBALS["mac"];
		$sql = mysqli_query($db,"select * from employees_data where Email='$Username' AND Password='$Password' AND MacAddress='$mac' AND StatusId=1 ");
		...
		if(mysqli_num_rows($sql) > 0)
		{
			...
			$sqll = mysqli_query($db,"select * from employee_data where RoleId='$roleid'");
